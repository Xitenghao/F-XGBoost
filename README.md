...
## config
* './da_config_gbdt.json', './da_config_xgb.json', and './da_config_mbr.json'define the parameter bounds for the DA hyperparameter tuning of the GBDT, XGBoost, and F-XGB models, respectively.
...
## DA for F-XGB
* './DA for F-XGB.ipynb' contains the process of optimizing the hyperparameters of the F-XGB model using the Dragonfly Algorithm.

## STT_test
* './STT_test.ipynb' contains the operation of the F-XGB model on the STT test set and the plotting program for Figure 8 in the manuscript.

## SHAP for F-XGB
* './SHAP for F-XGB.ipynb' contains the SHAP analysis process for F-XGB and the plotting program for Figure 9 in the manuscript.
